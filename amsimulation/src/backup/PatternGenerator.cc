...
int PatternGenerator::generate(TChain* TT, int* evtIndex, int evtNumber, int* nbTrackUsed, SectorTree* sectors,
                               map<int,pair<float,float> > eta_limits, int* coverageEstimation){
  ...
  while(nbModuleOk<evtNumber && (*evtIndex)<n_entries_TT){
    ...
    **************************************/
    ...
    for(unsigned int j=0;j<tracker_layers.size();j++){
      if(layers[j]==-1){
        missing_stub=true;
        if(sectors->getNbSectors()==1 && current_eta!=-10){ // we can use fake superstrips if we know the sector in which to add the tracks and
    ...
    */
    ...
    for(unsigned int j=0;j<tracker_layers.size();j++){
      ...
      if(stub_number==-2){//creation of a fake superstrip
        ...
        module = sector->getModuleCode(tracker_layers[j], CMSPatternLayer::getLadderCode(tracker_layers[j],m_stub_ladder[stub_number]),
                                       CMSPatternLayer::getModuleCode(tracker_layers[j],m_stub_module[stub_number]));

        ladder = sector->getLadderCode(tracker_layers[j],CMSPatternLayer::getLadderCode(tracker_layers[j],m_stub_ladder[stub_number]));

        //	cout<<"Layer "<<tracker_layers[j]<<" ladder "<<m_stub_ladder[stub_number]<<" module "<<m_stub_module[stub_number];
        //      <<" devient ladder "<<ladder<<" module "<<module<<endl;
        ...
        seg =  CMSPatternLayer::getSegmentCode(tracker_layers[j], CMSPatternLayer::getLadderCode(tracker_layers[j],m_stub_ladder[stub_number]),
                                               m_stub_seg[stub_number]);
      }

      /*
      cout<<"Event "<<*evtIndex<<endl;
      cout<<"    Layer "<<m_stub_layer[stub_number]<<" segment "<<seg<<" module "
          <<CMSPatternLayer::getModuleCode(m_stub_layer[stub_number],m_stub_module[stub_number])
          <<" ladder "<<m_stub_ladder[stub_number]<<" strip "<<strip<<endl;
